...
export function TreeExplorer({ open, onToggle, onFocusNode }: { open: boolean; onToggle: () => void; onFocusNode: (id: number) => void }) {
  ...
  return (
    <>
      <button
        onClick={onToggle}
        className="absolute z-30 w-6 h-10 flex items-center justify-center backdrop-blur-xl border border-[rgb(var(--fg-rgb)/0.08)] rounded-r-md text-[color:rgb(var(--fg-rgb)/0.5)] hover:text-[color:rgb(var(--fg-rgb))] hover:bg-[rgb(var(--fg-rgb)/0.05)] transition-all duration-300"
      ...
      >
        {/* Trees */}
        <div className="px-4 py-2.5 border-b border-[rgb(var(--fg-rgb)/0.07)] flex items-center justify-between">
          <span className="text-[9px] font-bold tracking-[0.18em] text-[color:rgb(var(--fg-rgb)/0.4)]">TREES</span>
          <span className="text-[10px] font-mono text-[color:rgb(var(--fg-rgb)/0.35)]">{treeIds.length}</span>
        </div>
        <div className="px-2 py-1.5 space-y-0.5 border-b border-[rgb(var(--fg-rgb)/0.06)]">
          {treeIds.length === 0 ? (
            <div className="text-[10px] font-mono text-[color:rgb(var(--fg-rgb)/0.25)] italic px-2 py-1">no trees</div>
          ) : (
            treeIds.map((id) => (
        ...
        <div className="px-4 py-2 flex items-center justify-between">
          <span className="text-[9px] font-bold tracking-[0.18em] text-[color:rgb(var(--fg-rgb)/0.4)]">NODES</span>
          <span className="text-[10px] font-mono text-[color:rgb(var(--fg-rgb)/0.35)]">{blueprint?.nodes.length ?? 0}</span>
        </div>
...
function TreeRow({ id, active, onSelect }: { id: string; active: boolean; onSelect: () => void }) {
  ...
  return (
    <button onClick={onSelect}
      className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg transition-colors text-left hover:bg-[rgb(var(--fg-rgb)/0.04)]"
      ...
      <GitBranch className="w-3.5 h-3.5 shrink-0" style={{ color: active ? '#06b6d4' : 'rgb(var(--fg-rgb) / 0.4)' }} />
      <span className={`text-[12px] font-semibold truncate ${active ? 'text-[color:rgb(var(--fg-rgb))]' : 'text-[color:rgb(var(--fg-rgb)/0.6)]'}`}>{id}</span>
    </button>
  );
}
...
function NodeRow({ node, onFocus }: { node: BTNodeDef; onFocus: () => void }) {
  ...
  return (
    <button onClick={onFocus}
      className="w-full flex items-center gap-2 px-2 py-1 rounded-md transition-colors text-left hover:bg-[rgb(var(--fg-rgb)/0.04)]"
      style={{ background: selected ? 'rgb(var(--fg-rgb) / 0.07)' : 'transparent' }}>
      <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${STATUS_DOT[status]}`} />
      <span className="text-[11px] font-medium text-[color:rgb(var(--fg-rgb)/0.8)] truncate">{node.name}</span>
      <span className="text-[9px] font-mono text-[color:rgb(var(--fg-rgb)/0.3)] ml-auto truncate max-w-[40%]">{node.type}</span>
    </button>
  );
}
